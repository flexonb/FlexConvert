...
export default function ImageOperationDialog({
  ...
  const steps: Step[] = useMemo(() => {
    const order: Array<"configure" | "preview" | "confirm"> = ["configure", "preview", "confirm"];
    const statusFor = (id: "configure" | "preview" | "confirm") => {
      const idx = order.indexOf(id);
      const curIdx = order.indexOf(currentStep);
      if (idx < curIdx) return "complete" as const;
      if (idx === curIdx) return "current" as const;
      return "upcoming" as const;
    };
    return [
      { id: "configure", label: operation === "crop" ? "Crop" : "Configure", status: statusFor("configure") },
      { id: "preview", label: "Preview", status: statusFor("preview") },
      { id: "confirm", label: "Confirm", status: statusFor("confirm") }
    ];
  ...
  function defaultOptionsFor(op: ConfigurableOperation): AnyOptions {
    switch (op) {
      case "resize":
        return { maxWidth: 1920, maxHeight: 1080, format: "jpeg", quality: 0.9 };
      case "crop":
        return { area: { x: 0, y: 0, width: 0, height: 0 }, format: "jpeg", quality: 0.92 };
      case "compress":
        return { format: "jpeg", quality: 0.7 };
      case "rotate":
        return { degrees: 90, format: "jpeg", quality: 0.9 };
      case "flip":
        return { horizontal: true, vertical: false, format: "jpeg", quality: 0.9 };
      case "convert":
        return { format: "webp", quality: 0.92 };
      case "grayscale":
        return {};
      case "adjust":
        return { brightness: 1.1, contrast: 1.05, saturation: 1.05, format: "jpeg", quality: 0.9 };
      case "text-overlay":
        return { text: "FlexConvert", opacity: 0.75, format: "jpeg", quality: 0.92 };
      default:
        return {};
    }
  }
  ...
  async function generatePreview() {
    if (!firstFile) {
      toast({
        ...
        description: "Please select an image to preview.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      if (operation === "crop") {
        ...
        currentOptions = { ...(options as CropOptions), area: croppedAreaPixels };
        setOptions(currentOptions);
      }

      switch (operation) {
        case "resize":
          blob = await resizeImage(firstFile, currentOptions as ResizeOptions);
          break;
        case "crop":
          blob = await cropImage(firstFile, currentOptions as CropOptions);
          break;
        case "compress":
          blob = await compressImage(firstFile, currentOptions as CompressOptions);
          break;
        case "rotate":
          blob = await rotateImage(firstFile, currentOptions as RotateOptions);
          break;
        case "flip":
          blob = await flipImage(firstFile, currentOptions as FlipOptions);
          break;
        case "convert":
          blob = await convertFormat(firstFile, currentOptions as ConvertOptions);
          break;
        case "grayscale":
          blob = await grayscaleImage(firstFile);
          break;
        case "adjust":
          blob = await adjustImage(firstFile, currentOptions as AdjustOptions);
          break;
        case "text-overlay":
          if (!(currentOptions as TextOverlayOptions).text?.trim()) {
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  inputMode="numeric"
                  value={o.maxWidth ?? ""}
                  onChange={(e) => setOptions({ ...o, maxWidth: numberOrUndefined(e.target.value) })}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  inputMode="numeric"
                  value={o.maxHeight ?? ""}
                  onChange={(e) => setOptions({ ...o, maxHeight: numberOrUndefined(e.target.value) })}
                />
              </div>
            </div>
            <OutputFormatQuality
        ...
        return (
          <div className="space-y-4">
            <LabeledSlider
              ...
              max={180}
              step={5}
              onChange={(v) => setOptions({ ...o, degrees: v })}
              format={(v) => `${v}°`}
            />
            <OutputFormatQuality
              format={o.format ?? "jpeg"}
      ...
      case "adjust": {
        ...
        return (
          <div className="space-y-4">
            <LabeledSlider
              label="Brightness"
              value={o.brightness ?? 1}
              min={0.5}
              max={1.5}
              step={0.05}
              onChange={(v) => setOptions({ ...o, brightness: v })}
            />
            <LabeledSlider
              label="Contrast"
              value={o.contrast ?? 1}
              ...
              max={1.5}
              step={0.05}
              onChange={(v) => setOptions({ ...o, contrast: v })}
            />
            <LabeledSlider
              label="Saturation"
              value={o.saturation ?? 1}
              min={0.5}
              max={1.5}
              step={0.05}
              onChange={(v) => setOptions({ ...o, saturation: v })}
            />
            <OutputFormatQuality
              format={o.format ?? "jpeg"}
              quality={o.quality ?? 0.9}
        ...
        return (
          <div className="space-y-4">
            ...
            <LabeledSlider
              ...
              max={1}
              step={0.05}
              onChange={(v) => setOptions({ ...o, opacity: v })}
      ...
      <DialogContent className="sm:max-w-2xl">
          ...
          {currentStep === "confirm" && (
            <div className="space-y-3">
              <div className="text-sm text-gray-600 dark:text-gray-300">
                You are about to process {files.length} file{files.length > 1 ? "s" : ""} with this configuration:
              </div>
              <pre className="text-xs p-3 rounded-lg bg-gray-50 dark:bg-gray-800 border border-gray-200/50 dark:border-gray-700/50 overflow-auto max-h-40">
        ...
        <DialogFooter className="flex items-center justify-between">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {currentStep === "preview" && (
              previewUrl ? (
                <Button onClick={() => setCurrentStep("confirm")}>Continue</Button>
              ) : (
